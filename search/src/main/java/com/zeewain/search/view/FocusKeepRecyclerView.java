    ...
    public View focusSearch(View focused, int direction) {
        ...
        if (view != null) {
            ...
            if (nextFocusItemView == null) {
                ...
                Log.d(TAG, "focusSearch mFocusLostListener");
                //调用移出的监听
...
//将当前的view放到Focusable views列表中，再次移入焦点时会取到该view,实现焦点记忆功能
    ...
    public boolean requestFocus(int direction, Rect previouslyFocusedRect) {
        Log.d(TAG, "mCurrentFocusPosition: " + mCurrentFocusPosition);
        View lastFocusedView = getLayoutManager().findViewByPosition(mCurrentFocusPosition);
