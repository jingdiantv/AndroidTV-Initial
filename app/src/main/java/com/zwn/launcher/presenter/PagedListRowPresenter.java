...
public class PagedListRowPresenter extends BaseListRowPresenter {
    private final LifecycleOwner lifecycleOwner;
    private final String categoryId;
    private MainViewModel viewModel;

    public PagedListRowPresenter(LifecycleOwner lifecycleOwner, String categoryId){
            ...
            if(event.getKeyCode() == KeyEvent.KEYCODE_DPAD_RIGHT && event.getAction() == KeyEvent.ACTION_UP){
                ...
                if(horizontalGridView.getSelectedPosition() > 0) {
                    ...
                    if (gridViewLayoutManager != null) {
                        ...
                        if(itemCount < viewModel.getTotalRecordSize(categoryId)) {
                            if (horizontalGridView.getSelectedPosition() >= itemCount - 3) {
                                viewModel.reqProductList(categoryId);
                            }
                        }
                    }
                }
                ...
                if(gridViewLayoutManager != null && horizontalGridView.getScrollState() == RecyclerView.SCROLL_STATE_IDLE){
                    int itemCount = gridViewLayoutManager.getItemCount();
                    if(horizontalGridView.getSelectedPosition() == (itemCount -1) && itemCount == viewModel.getTotalRecordSize(categoryId)){
    ...
    protected void onBindRowViewHolder(RowPresenter.ViewHolder holder, Object item) {
        ...
        PagedListRow listRow = (PagedListRow) item;
        ...
        if(pagedListRowPresenterViewHolder.getAdapter() == null){
            ...
            viewModel.mldProductListLoadState.observe(lifecycleOwner, pagedPrdListLoadState -> {
                if(categoryId.equals(pagedPrdListLoadState.categoryId)) {
                    if(pagedPrdListLoadState.pageNum > 1){
                        if(pagedPrdListLoadState.loadState == LoadState.Loading){
                        ...
                        }else if(pagedPrdListLoadState.loadState == LoadState.Success) {
                            String skuId = ((ProductListMo.Record)adapter.get(adapter.size() -1)).getSkuId();
                            if(ProdConstants.SKU_ID_LOADING.equals(skuId) || ProdConstants.SKU_ID_LOADED_ERR.equals(skuId)){
                                adapter.removeItems(adapter.size() -1, 1);
                            }
                            List<ProductListMo.Record> recordList = viewModel.getProductListFromCache(categoryId).getRecords();
                            ...
                            //HorizontalGridView horizontalGridView = ((ListRowView)holder.view).getGridView();
                            //horizontalGridView.setSelectedPositionSmooth(size);
                        }else{
                            String skuId = ((ProductListMo.Record)adapter.get(adapter.size() -1)).getSkuId();
